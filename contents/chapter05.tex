...
Experiment~5 replaces the hand-crafted feature pipeline by a learned graph-based feature extractor. The Adaptive Graph Convolutional Network described in Section~\ref{sec:joint_corr_agcn} is applied to the bilateral skeleton representation: the adaptive graph is derived from the layer-wise feature transformation through a Mahalanobis distance and Gaussian kernel, allowing the model to learn sample-dependent joint relations from the input representation. After graph propagation, the per-node embeddings are average-pooled into a graph-level representation and passed to a linear classification head. Henceforth this configuration is referred to simply as \emph{AGCN}.

To directly mirror the method comparison shown in the oral defense, the AGCN result is reported alongside the strongest baselines from Experiments~2--4: the bilateral XGB Importance baseline (Exp.~2), the left-hand XGB Importance baseline (Exp.~3), and the three correlation-augmented configurations (Exp.~4). All baselines use XGBoost as the downstream classifier.

\subsection{Results}

Table~\ref{tab:gcn_methods_2020} reports the 2020 within-cohort method comparison; Table~\ref{tab:gcn_methods_2025} reports the 2025 within-cohort method comparison; and Table~\ref{tab:gcn_cross_dataset} reports the 2020-to-2025 cross-cohort transfer comparison. Figure~\ref{fig:gcn_roc_comparison} shows the corresponding ROC curves for AGCN.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}XYYY}
        ...
        Method & AUROC & Accuracy & F1-score \\
        \hline
        Left XGB Importance                  & 0.66 & 0.67 & 0.48 \\
        XGB Importance                       & 0.74 & 0.78 & 0.60 \\
        Correlation + ANOVA                  & 0.74 & 0.77 & 0.63 \\
        Correlation + Logistic L1            & 0.73 & 0.81 & 0.63 \\
        Correlation + XGB Importance         & 0.74 & 0.78 & 0.62 \\
        AGCN                                 & 0.98 & 0.90 & 0.84 \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}XYYY}
        ...
        Method & AUROC & Accuracy & F1-score \\
        \hline
        Left-hand XGB Importance             & 0.75 & 0.80 & 0.63 \\
        XGB Importance                       & 0.69 & 0.72 & 0.54 \\
        Correlation + ANOVA                  & 0.61 & 0.63 & 0.44 \\
        Correlation + Logistic L1            & 0.55 & 0.60 & 0.42 \\
        Correlation + XGB Importance         & 0.70 & 0.65 & 0.50 \\
        AGCN                                 & 0.98 & 0.92 & 0.84 \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}XYYY}
        ...
        \hline
        Left XGB Importance                  & 0.66 & 0.67 & 0.48 \\
        XGB Importance                       & 0.65 & 0.79 & 0.47 \\
        Correlation + ANOVA                  & 0.68 & 0.75 & 0.49 \\
        Correlation + Logistic L1            & 0.66 & 0.61 & 0.47 \\
        Correlation + XGB Importance         & 0.68 & 0.69 & 0.50 \\
        AGCN                                 & 0.59 & 0.39 & 0.39 \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{RMSE curves of AGCN across training epochs.}
    \label{fig:gcn_rmse_comparison}
\end{figure}

\subsection{Observation}

Within cohort, AGCN dominates every hand-crafted configuration from Experiments~2--4, reaching AUROCs of $0.98$ on both the 2020 and 2025 cohorts with accuracies of $0.90$--$0.92$ and F1-scores of $0.84$. This indicates that adaptive graph learning captures discriminative joint-wise motion patterns that are not fully represented by the autocorrelation, amplitude, or pairwise-correlation feature sets in earlier experiments.

In the cross-cohort transfer setting, however, AGCN drops to an AUROC of $0.59$, an accuracy of $0.39$, and an F1-score of $0.39$---falling behind every hand-crafted configuration in Table~\ref{tab:gcn_cross_dataset}. This sharp reversal suggests that the learned graph topology is highly dependent on the training distribution. When the recording protocol or patient population shifts from the 2020 cohort to the 2025 cohort, the graph structure learned on the source cohort does not transfer reliably, exacerbating the domain-shift problem typical of small clinical datasets.
