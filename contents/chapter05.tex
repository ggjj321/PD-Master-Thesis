...
The left-hand XGB Importance + XGBoost configuration is the strongest single-hand baseline, reaching AUROCs of $0.76$ and $0.75$ on the 2020 and 2025 cohorts respectively, with accuracies of $0.82$ and $0.81$. The advantage is most consistent for the XGB-importance selector: it outperforms the corresponding right-hand configuration in both cohorts and also yields the highest cross-cohort F1-score among single-hand models. This pattern, together with the left-heavy top-feature distribution observed in Experiment~2, is qualitatively compatible with the motor-reserve hypothesis discussed in Section~\ref{sec:rw_motor_reserve}. Because individual handedness was not recorded, the finding is interpreted as an empirical left-versus-right asymmetry rather than direct evidence that the left hand was non-dominant for all subjects.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}XYYY}
        ...
        Correlation + XGB Importance + XGBoost      & 0.74 / 0.70 & 0.78 / 0.65 & 0.62 / 0.51 \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}XYYY}
        ...
        Correlation + ANOVA + XGBoost               & 0.68 & 0.56 & 0.45 \\
        Correlation + L1-Logistic + XGBoost         & 0.67 & 0.64 & 0.43 \\
        Correlation + XGB Importance + XGBoost      & 0.68 & 0.58 & 0.45 \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}XYYY}
        ...
        Correlation + XGB Importance         & 0.70 & 0.65 & 0.51 \\
        AGCN                                 & 0.98 & 0.92 & 0.85 \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}XYYY}
        ...
        Correlation + ANOVA                  & 0.68 & 0.56 & 0.45 \\
        Correlation + Logistic L1            & 0.67 & 0.64 & 0.43 \\
        Correlation + XGB Importance         & 0.68 & 0.58 & 0.45 \\
        AGCN                                 & 0.59 & 0.46 & 0.40 \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{ROC curves of AGCN under within-cohort and cross-cohort evaluation.}
...
Within cohort, AGCN dominates every hand-crafted configuration from Experiments~2--4, reaching AUROCs of $0.98$ on both the 2020 and 2025 cohorts with accuracies of $0.92$--$0.93$ and F1-scores of $0.85$--$0.89$. This indicates that adaptive graph learning captures discriminative joint-wise motion patterns that are not fully represented by the autocorrelation, amplitude, or pairwise-correlation feature sets in earlier experiments.

In the cross-cohort transfer setting, however, AGCN drops to an AUROC of $0.59$, an accuracy of $0.46$, and an F1-score of $0.40$---falling behind every hand-crafted configuration in Table~\ref{tab:gcn_cross_dataset}. This sharp reversal suggests that the learned graph topology is highly dependent on the training distribution. When the recording protocol or patient population shifts from the 2020 cohort to the 2025 cohort, the graph structure learned on the source cohort does not transfer reliably, exacerbating the domain-shift problem typical of small clinical datasets.
