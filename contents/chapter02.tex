...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.22\linewidth}
    ...
    \hline
    Output granularity    & Binary (PD vs.\ HS) & 0--4 scale (5 items) \\
    ...
    Pronation Supination task  binary PD vs HS  & AUROC 0.944, accuracy 0.917 (LR)  & AUROC 0.75, accuracy 0.73 (RF)\\
    \hline
  \end{tabularx}
\end{table}
...
MediaPipe Hands instantiates this framework with a two-stage hand tracking solution optimised for real-time, on-device execution. Given an input red-green-blue (RGB) frame, a lightweight palm detector first localises the hand region; the corresponding image crop is then passed to a keypoint regression model that returns 21 anatomically defined hand landmarks with normalised $(x,y,z)$ coordinates per frame. The $x$ and $y$ components are normalised by image dimensions, while the $z$ value encodes approximate depth relative to the wrist joint, enabling three-dimensional trajectory analysis without a dedicated depth sensor. Inference runs entirely on the central processing unit (CPU) or graphics processing unit (GPU) of a consumer device, with the framework's scheduler managing parallel execution across calculators to sustain real-time frame rates.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.35\linewidth} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
        ...
        Maintenance   & Actively maintained by Google         & Open-source community \\
...
This principle carries a specific implication for bilateral hand assessment in PD. If a cohort is predominantly right-handed, the dominant hand would be expected to benefit from higher accumulated motor reserve and may exhibit compensated, attenuated motor deficits at a given stage of pathology. The non-dominant hand, carrying less reserve, may manifest bradykinesia more saliently and at an earlier disease stage. Because subject-level handedness labels are not available in the present dataset, this mechanism is treated as a plausible interpretive hypothesis rather than direct evidence that the left hand is non-dominant for every participant. Table~\ref{tab:motor_reserve_summary} summarises this behavioural contrast.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.30\linewidth} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
        \hline
        Aspect & Dominant hand (often right) & Non-dominant hand (often left) \\
        \hline
...
This asymmetry provides one qualitative explanation for the empirically stronger and more transferable classification performance of left-hand features observed in Chapter~\ref{chap:results}. In the absence of individual handedness data, however, the result should be interpreted conservatively as a left-versus-right lateralisation pattern that is compatible with motor reserve, not as proof of non-dominant-hand impairment in every subject.
